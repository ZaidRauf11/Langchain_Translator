...
# 1. Give a short definition (3–4 lines).
# 2. Identify all important words (especially nouns, adjectives, verbs).
# 3. For **each word**, return at least 1 synonym and 1 antonym.
...
# If any part is not applicable, still try your best to fill synonyms/antonyms.
# """),
...
# load_css("langchain_translator_gemini.css")
...
# # word_prompt for translating each individual word in a sentence
# word_prompt = ChatPromptTemplate.from_messages([
#     ("system", "Translate each word from {input_language} to {output_language} in this sentence:"),
#     ("human", "{input}")
# ])

# # extra_prompt for better antonym and word breakdown support
# extra_prompt = ChatPromptTemplate.from_messages([
#     ("system", "You are a language assistant. When given an English word or sentence, provide a short definition (3–4 lines), then identify all key words — especially nouns, verbs, and adjectives. For each, specify the word type and include at least one synonym and one antonym. Format your response clearly with a 'Definition' section followed by a 'Vocabulary' section listing each word, its type, synonyms, and antonyms. If some words are difficult, still try to provide meaningful matches."),
#     ("human", "{input}")
# ])
...
# # Streamlit UI
# st.title('🧠 Smart Translator using Gemini & Langchain')
